  ...
  run: async (client, interaction) => {
    ...
    if (dtime !== null && timeout - (Date.now() - dtime) > 0) {
      ...
      const timebed = new MessageEmbed()
        ...
        .setDescription(`You have already claimed your daily reward for the day! Come back in about ${time.hours}h ${time.minutes}m ${time.seconds}s`);
      ...
      if (jack == RNG) {
        ...
        if (!storage) {
          ...
          client.db.set(`daily_${interaction.user.id}.${interaction.guild.id}`, Date.now());
          client.db.add(`bal_${interaction.user.id}.${interaction.guild.id}`, regular);
        }
      } else {
