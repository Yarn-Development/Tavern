 ...
 */
	run: async (client, interaction) => {
		...
		if (!(newuser == null)) {
			...
			const yncoll = interaction.channel.createMessageComponentCollector({ componentType: 'BUTTON', time: 15000 }); // respond to a button being clicked
			yncoll.on('collect', (i) => {
				if (i.customId === 'YesButton') { // if the button clicked is the YesButton
					if (i.user.id === interaction.user.id) {
						interaction.followUp({ content: 'As you wish Traveller!', embeds: [infobed], ephemeral: true });
					}
					else {
						interaction.followUp({ content: 'Sorry, but the bartender has not visited you, but you can pay a visit to him via the /bartender command.' }); // i dont know if i need to do this for all of them, needs to be debugged
					}
				}
			...
			const infochoice = new MessageButton()
				.setCustomId('RuleButton').setLabel('Introduction').setStyle('SUCCESS');
			const confchoice = new MessageButton()
				.setCustomId('SkipRules').setLabel('Skip to Configuration').setStyle('DANGER');
			interaction.followUp({
				embeds: [welcome],
				components: [new MessageActionRow().addComponents(confchoice, infochoice)],
			});
			...
			rulecollector.on('collect', (i) => {
				...
				if (i.customId === 'RuleButton') {
					const nextbtn = new MessageButton()
						.setCustomId('next').setLabel('Continue to Configuration').setStyle('PRIMARY');
					interaction.followUp({
						embeds: [infobed],
					...
					cfgselector.on('collect', (f) => {
						...
						if (f.customId === 'next') {
							const globalbtn = new MessageButton()
								.setCustomId('GlobalButton').setLabel('Global Economy').setStyle('PRIMARY');
							const guildbtn = new MessageButton()
								.setCustomId('GuildButton').setLabel('Per-Guild Economy').setStyle('PRIMARY');
							interaction.followUp({
								...
								components: [new MessageActionRow().addComponents(guildbtn, globalbtn)],
							});
